...
class AdvancedOptimizedRaceController:
    ...
    def __init__(self, school_name="安徽医科大学", team_name="杏林方舟队"):
        ...
        # 调试输出频率
        self.debug_update_interval = 10  # 每10帧更新一次调试显示
    ...
    def set_servo_angle(self, angle):
        ...
        if abs(angle - self.current_servo_angle) < 5:
            return
    ...
    def detect_lane_lines(self, frame):
        ...
        roi_vertices = np.array([[(0, 350), (0, 100), (600, 100), (600, 350)]], dtype=np.int32)
        roi_mask = np.zeros_like(gray)
        cv2.fillPoly(roi_mask, roi_vertices, 255)
        masked_edges = cv2.bitwise_and(gray, roi_mask)

        tophat = cv2.morphologyEx(masked_edges, cv2.MORPH_TOPHAT, self.kernel_rect_30x1)
        blur = cv2.GaussianBlur(tophat, (7, 7), 0)
        canny = cv2.Canny(blur, 50, 100)

        pts = np.array([[[10, 220], [10, 150], [590, 150], [590, 220]]], dtype=np.int32)
        mask = np.zeros_like(gray)
        cv2.fillPoly(mask, pts, color=255)
        image_masked = cv2.bitwise_and(mask, canny)

        del tophat, blur, canny, mask, roi_mask, masked_edges

        lines = cv2.HoughLinesP(image_masked, 1, np.pi / 180,
                                threshold=30, minLineLength=50, maxLineGap=10)

        if lines is not None:
            left_lines = [line for line in lines if self.calculate_slope(line) < -0.3]
            right_lines = [line for line in lines if self.calculate_slope(line) > 0.3]

            with self.center_x_lock:
                self.center_x = self.get_lane_center(left_lines, right_lines)
    ...
    def detect_zebra_crossing(self, frame):
        ...
        roi = frame[int(height * 0.55):int(height * 0.85), :]  # 更聚焦底部
    ...
    def detect_cone(self, frame):
        ...
        h, w = frame.shape[:2]
        # 定义 ROI：y 从 250 到 380（底部附近），x 从 150 到 450（中间区域）
        y1, y2 = int(h * 0.625), int(h * 0.95)  # 250 ~ 380 (假设 h=400)
        x1, x2 = int(w * 0.25), int(w * 0.75)  # 150 ~ 450 (假设 w=600)
        ...
        for color, mask in cone_masks.items():
            mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, self.kernel_5x5)
            mask = cv2.morphologyEx(mask, cv2.MORPH_OPEN, self.kernel_5x5)
    ...
    def handle_normal_driving(self, frame):
        ...
        if not self.in_avoiding and (current_time - self.cone_detected_time) > self.cone_avoid_cooldown:
            if self.should_check('cone'):
                ...
                if cone_detected and cone_area > 400:
        ...
        try:
            while True:
                ...
                if self.debug_mode and self.frame_counter % self.debug_update_interval == 0:
                    ...
                    cv2.putText(frame, f"FPS: {fps:.1f} | Speed: {self.current_speed}", (10, 60),
