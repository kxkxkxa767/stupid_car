...
int main() 
{
	...
	VideoCapture cap("/home/5G/5G/main/lab/build/output.avi");
	...
	while (cap.read(Original_image))
	{
		...
		if(Start==3)
		{
			...
			//AdaptiveBlueMaskLab(Lab_image, Binary_image);
			...
			// 4. 膨胀操作
			Mat dilated;
			dilate(Mask, dilated, kernel, Point(-1, -1), 3);  // 1次迭代
			//imshow("dilated",dilated);
			// 3. 腐蚀操作
			Mat eroded;
			erode(dilated, eroded, kernel, Point(-1, -1), 1);  // 1次迭代

			dilate(eroded, dilated, kernel, Point(-1, -1), 1);

			erode(dilated, eroded, kernel, Point(-1, -1), 1);

			dilate(eroded, dilated, kernel, Point(-1, -1), 3);

			//压缩
			resize(dilated, Binary_image , Size(LCDW, LCDH), 0, 0, INTER_NEAREST);
			//resize(eroded, Binary_image , Size(LCDW, LCDH), 0, 0, INTER_NEAREST);
