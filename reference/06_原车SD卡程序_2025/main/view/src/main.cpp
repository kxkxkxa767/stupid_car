...
#define YunTai_Y 73       //云台Y轴初始位置//73
#define Steer_Center 99   //舵机中值//84
#define Steer_min 89      //舵机限幅 右转 min 62
#define Steer_max 109      //舵机限幅 左转 max 102   
...
std::deque<std::string> ab_results;
...
*/
	...
	else if(Block_Flag == 1 && Block_Done == 0 && !Blue_Block.empty())
	{
		...
		const int transition_total = 100; // 过渡帧数

		for (int Ysite = LCDH - 1; Ysite > ImageStatus.OFFLineBoundary + 1; Ysite--)
		{
			int center_normal = (ImageDeal[Ysite].LeftBoundary_First + ImageDeal[Ysite].RightBoundary_First) / 2;
			float alpha = (float)transition_step / transition_total;

			if (ImageStatus.Det_True > 0) // 误差大于0，插值左边界辅助
			{
				int center_left = (ImageDeal[Ysite].LeftBoundary_First + (ImageDeal[Ysite].LeftBoundary_First + ImageDeal[Ysite].RightBoundary_First) / 2) / 2;
				ImageDeal[Ysite].Center = (1.0f - alpha) * center_left + alpha * center_normal;
			}
			else if (ImageStatus.Det_True < 0) // 误差小于0，插值右边界辅助
			{
				int center_right = ((ImageDeal[Ysite].LeftBoundary_First + ImageDeal[Ysite].RightBoundary_First) / 2 + ImageDeal[Ysite].RightBoundary_First) / 2;
				ImageDeal[Ysite].Center = (1.0f - alpha) * center_right + alpha * center_normal;
			}
			else // 误差为0，直接用原始中心线
			{
				ImageDeal[Ysite].Center = center_normal;
			}
		}
		...
		if (transition_step < transition_total)
			...
			transition_step = transition_total;

		// 避障完成后重置
		if (Block_Done == 1)
			transition_step = 0;
	}
	else if(Zebra_Flag == 4 && Stop_Flag == 0)
	{
		static int right_offset = 0; // 右边界累计偏移量
		const int target_offset = 10; // 目标偏移量

		// 每20帧调整一次
		if (Slow_num % 20 == 0 && right_offset < target_offset) {
			right_offset += 2;
			if (right_offset > target_offset) right_offset = target_offset;
		}

		for (int Ysite = LCDH - 1; Ysite > ImageStatus.OFFLineBoundary + 1; Ysite--)
		{
			if(Stop_left_right == 1) // 左停车
			{
				if (right_offset < target_offset) {
					// 右边界逐步左移
					ImageDeal[Ysite].RightBoundary_First -= right_offset;
				} else {
					// 达到目标后，直接用中心线做右边界
					ImageDeal[Ysite].RightBoundary_First = ImageDeal[Ysite].Center;
				}
				// 中线正常
				ImageDeal[Ysite].Center = (ImageDeal[Ysite].LeftBoundary_First + ImageDeal[Ysite].RightBoundary_First) / 2;
			}
			else if(Stop_left_right == 0) // 右停车，左边界右移
			{
				static int left_offset = 0;
				if (Slow_num % 20 == 0 && left_offset < target_offset) {
					left_offset += 2;
					if (left_offset > target_offset) left_offset = target_offset;
				}
				if (left_offset < target_offset) {
					ImageDeal[Ysite].LeftBoundary_First += left_offset;
				} else {
					ImageDeal[Ysite].LeftBoundary_First = ImageDeal[Ysite].Center;
				}
				ImageDeal[Ysite].Center = (ImageDeal[Ysite].LeftBoundary_First + ImageDeal[Ysite].RightBoundary_First) / 2;
			}
		}
	...
	/*
	else if(Zebra_Flag == 5 && Stop_Flag == 0)
	{
		static int right_offset = 0; // 右边界累计偏移量
		const int target_offset = 10; // 目标偏移量
...
vector<Rect> YellowBlock() 
{
    ...
    Scalar lower_Y(10, 50, 100);   // H:20 S:100 V:100
...
void DetectParkingArea()
{
    ...
    // 常用黄色 HSV 范围（可根据实际场景调节）
    Scalar lower_Y(10, 10, 100);   // H:20 S:100 V:100
    Scalar upper_Y(35, 255, 255);   // H:35 S:255 V:255
...
bool DetectZebraCrossing()
{
    ...
    Mat thresh_zebra;
    threshold(blur_zebra, thresh_zebra, 180, 255, THRESH_BINARY);
    ...
    for (int Ysite = 60; Ysite < 95; Ysite++) // 400高，60~120约为1/6~1/3高度
    {
        for (int Xsite = 80; Xsite < 720; Xsite++) // 800宽，左右各缩进80
        {
		...
		if (Lane_Change_Flag == 0)
		{
			if (Lane_Change == 1)
			{
				if (!Right_Change && fabs(right_slope - last_right_slope) >= 0.2)
					Right_Change = true;
				if (Right_Change && !Left_Change && fabs(left_slope - last_left_slope) >= 0.2)
					Left_Change = true;
			}
			else if (Lane_Change == 3)
			{
				if (!Left_Change && fabs(left_slope - last_left_slope) >= 0.2)
					Left_Change = true;
				if (Left_Change && !Right_Change && fabs(right_slope - last_right_slope) >= 0.2)
					Right_Change = true;
			}
		}
		// 左变道
...
std::string recognizeAB_and_vote()
{
    ...
    Mat hsv;
    cvtColor(Cropped_image, hsv, COLOR_BGR2HSV);
    int h_low1 = 0, h_high1 = 20, s_low1 = 70, v_low1 = 170, v_high1 = 255;
    int h_low2 = 160, h_high2 = 180, s_low2 = 5, v_low2 = 170, v_high2 = 255;
    Scalar lower_red1(h_low1, s_low1, v_low1), upper_red1(h_high1, 255, v_high1);
    Scalar lower_red2(h_low2, s_low2, v_low2), upper_red2(h_high2, 255, v_high2);
...
std::string recognizeLeftRightByWhiteBar()
{
    ...
    Mat hsv;
    cvtColor(Cropped_image, hsv, COLOR_BGR2HSV);
    int h_low1 = 0, h_high1 = 13, s_low1 = 90, v_low1 = 100, v_high1 = 255;
    int h_low2 = 160, h_high2 = 180, s_low2 = 90, v_low2 = 100, v_high2 = 255;
    Scalar lower_red1(h_low1, s_low1, v_low1), upper_red1(h_high1, 255, v_high1);
    Scalar lower_red2(h_low2, s_low2, v_low2), upper_red2(h_high2, 255, v_high2);
    Mat mask1, mask2, mask;
    inRange(hsv, lower_red1, upper_red1, mask1);
...
int main() 
{
	...
	{ 
		...
		if(Start==3)
		{
			...
			for (int i = 0; i < out_h; ++i) {
				for (int j = 0; j < out_w; ++j) {
					...
					for (int k = 0; k < out_c; ++k) {
						val = max(val, static_cast<int>(y[idx + k]));
					}*/
			...
			//斑马线检测
			if(Zebra_Flag== 0 && Block_Done == 1 && SlowSpeed_Flag == 1)
			{
			...
			if(Zebra_Flag == 1)
			{
				...
				if(Time_num == 1)
				{
					system("amixer -c 2 sset 'PCM' 90%");
					system("aplay -D plughw:2,0 /home/5G/5G/voice/team.wav"); // 播放语音
				}
			...
			if(Zebra_Flag == 2)
			{
				...
				if(Time_num < 15)
				{
					...
					// 新增：提前判断L或R累计达到10次，直接Time_num=20
				...
				else
				{
					...
					for (const auto& r : lr_results) count[r]++;
			...
			if(Zebra_Flag == 3 && Lane_Change < 4)
			{
				...
				else if (Lane_Change == 1)
				{
					...
					else if(Yaw_step == 1)
					{
						...
						{
							Time_num++;
							int time_low = (Lane_Change_Flag == 0) ? 6 : 8;//右变道前，左变道后
							int time_high  = (Lane_Change_Flag == 0) ? 10 : 10;//右变道前，左变道后
				...
				else if(Lane_Change == 3)
				{
					...
					else if(Yaw_step == 1)
					{
						...
						{
							Time_num++;
							int time_low = (Lane_Change_Flag == 0) ? 4 : 8;//右变道前，左变道后
							int time_high  = (Lane_Change_Flag == 0) ? 8 : 15;//右变道前，左变道后
							if(Time_num >= time_low || limit_time >= time_high)
							{
								printf("Time_num:%d\n",Time_num);
								Yaw_step = 0;
			...
			if(Zebra_Flag == 4 && SlowSpeed_Flag == 2)
			{
				...
				if(Time_num < 15)
				{
					...
					// 新增：提前判断A或B累计达到10次，直接Time_num=20
				...
				else
				{
					// 统计最多的A/B
					std::map<std::string, int> count;
					for (const auto& r : ab_results) count[r]++;
			...
			if(Zebra_Flag == 5)
			{
				...
				if (!Yellow_Parking.empty() && Stop_Flag == 0 && ((Yellow_Parking.y + Yellow_Parking.height) >= 20))
				{
					Stop_Flag = 1;
					printf("Parking Area Detected!\n");
				}
				if(Stop_Flag == 1 && ((Yellow_Parking.y + Yellow_Parking.height) <= 100))
				{
					Stop_Flag = 2;
		...
		if(Start==4)
		{
			if (!trackbar_created)
			{
				...
				createTrackbar("YunTai_X", "Original_image", NULL, 180, NULL);
				createTrackbar("YunTai_Y", "Original_image", NULL, 180, NULL);
