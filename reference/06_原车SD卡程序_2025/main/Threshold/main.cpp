...
inline bool InImage(const Mat& img, int y, int x)
{
...
void Search_Bottom_Line_OTSU(Mat Image, int Row, int Col, int Bottonline)
{
	...
	//printf("LeftBoundary: %d  RightBoundary: %d\n", ImageDeal[Bottonline].LeftBoundary, ImageDeal[Bottonline].RightBoundary);
}

void Search_Left_and_Right_Lines(Mat Image, int Row, int Col, int Bottonline)
{
    //if (Image.empty() || Bottonline < 0 || Bottonline >= Row) return;
    ...
    int Pixel_Left_Xsite = Left_Xsite;
    ...
    int Pixel_Right_Xsite = Right_Xsite;
    ...
    while (1)
    {
        ...
        if ((Pixel_Left_Ysite > Ysite) || Ysite == ImageStatus.OFFLineBoundary)
        {
            ...
            if (!InImage(Image, Pixel_Left_Ysite, Pixel_Left_Xsite))
            {
                ImageStatus.OFFLineBoundary = Ysite;
                break;
            }
            ...
            else
            {
                ...
                if (Left_Ysite >= 0 && Left_Ysite < LCDH)
                {
        ...
        if ((Pixel_Right_Ysite > Ysite) || Ysite == ImageStatus.OFFLineBoundary)
        {
            ...
            if (!InImage(Image, Pixel_Right_Ysite, Pixel_Right_Xsite))
            {
                ImageStatus.OFFLineBoundary = Ysite;
                break;
            }
            ...
            else
            {
                ...
                if (Right_Ysite >= 0 && Right_Ysite < LCDH)
                {
...
void DetectParkingArea()
{
    ...
    // 常用黄色 HSV 范围（可根据实际场景调节）
    Scalar lower_Y(5, 10, 100);   // H:20 S:100 V:100
    Scalar upper_Y(45, 255, 255);   // H:35 S:255 V:255
	...
	if(frame_count >= 13)
	{
		if (Lane_Change_Flag == 0)
		{
			if (Lane_Change == 1)
			{
				...
				if (Right_Change && !Left_Change && fabs(left_slope - last_left_slope) >= 0.1)
					Left_Change = true;
			}
			else if (Lane_Change == 3)
			{
				if (!Left_Change && fabs(left_slope - last_left_slope) >= 0.2)
					Left_Change = true;
				if (Left_Change && !Right_Change && fabs(right_slope - last_right_slope) >= 0.1)
					Right_Change = true;
			}
		}
		// 左变道
...
int main() 
{
    ...
    int bilateral_d = 12;      // 邻域直径//9
    int bilateral_sigmaColor = 200; // 颜色空间标准差//50
    int bilateral_sigmaSpace = 40; // 坐标空间标准差//40


    namedWindow("edges", WINDOW_NORMAL);
    createTrackbar("d", "edges", &bilateral_d, 20);
    createTrackbar("sigmaColor", "edges", &bilateral_sigmaColor, 255);
    createTrackbar("sigmaSpace", "edges", &bilateral_sigmaSpace, 255);
...
/*
	createTrackbar("canny_low", "Road_image", &canny_low, 255);
	createTrackbar("canny_high", "Road_image", &canny_high, 255);
*/
	...
	//VideoCapture cap("/home/5G/5G/OpenCV/new4.avi");
	//VideoCapture cap("/home/5G/5G/main/Threshold/build/bd.mp4");
	...
	while (cap.read(Original_image))
	{
		...
		//灰度
		cvtColor(Cropped_image, Gray_image, COLOR_BGR2GRAY);

		resize(Gray_image, Road_image, Size(LCDW*2, LCDH*2));
		//resize(Gray_image, Road_image, Size(LCDW, LCDH));
		...
		{
			//双边滤波
			int d = bilateral_d > 0 ? bilateral_d : 1; // d不能为0
        	Mat Bilateral_image;
        	bilateralFilter(Road_image, Bilateral_image, d, bilateral_sigmaColor, bilateral_sigmaSpace);

			//高斯滤波
			Mat gaussian_blur;
        	GaussianBlur(Bilateral_image, gaussian_blur, cv::Size(5, 5), 0);

			//Canny边缘检测
			Mat edges;
			int low = canny_low > 0 ? canny_low : 1;
			int high = canny_high > 0 ? canny_high : 1;
			Canny(gaussian_blur, edges, low, high);

			//膨胀
			Mat kernel = getStructuringElement(MORPH_RECT, Size(3, 3));
			Mat dilate_img;
			dilate(edges, dilate_img, kernel, Point(-1, -1), 1);

			//中值滤波
			//Mat median_img;
			//medianBlur(dilate_img, median_img, 5); // 3为核大小，可调

			//腐蚀
			//Mat morph_img;
			//erode(dilate_img, morph_img, kernel, Point(-1, -1), 1);

			vector<Vec4i> lines;
			HoughLinesP(dilate_img, lines, 1, CV_PI / 180, 50, 30, 10);
			//HoughLinesP(median_img, lines, 1, CV_PI / 180, 50, 30, 10);
			//HoughLinesP(morph_img, lines, 1, CV_PI / 180, 50, 30, 10);
			binary = Mat::zeros(Road_image.size(), CV_8UC1);
			for (size_t i = 0; i < lines.size(); i++)
			{
				...
				if (fabs(angle) > 10.0) // 只画角度绝对值大于15度的线
				{
					line(binary, cv::Point(l[0], l[1]), cv::Point(l[2], l[3]), Scalar(255), 4);
				}
			}
			resize(binary, Binary_image, Size(LCDW, LCDH));
			threshold(Binary_image, Binary_image, 50, 255, THRESH_BINARY);
				...
				if(!Blue_Block.empty() && Block_Flag == 0)
				{
						...
						Blue_Block_Ysite > 20 && Blue_Block_Ysite< 110)
					{
						...
						*/
						...
						printf("center:%d\n",((ImageDeal[Blue_Block_Ysite].LeftBoundary_First + ImageDeal[Blue_Block_Ysite].RightBoundary_First) / 2-20));
				...
				else if(Lane_Change == 3)
				{
					...
					else if(Yaw_step == 1)
					{
						...
						if (abs(CenterLineSlope) <= 0.7 && !(abs(LeftBoundarySlope) <= 0.05 && abs(RightBoundarySlope) <= 0.05))
						{
							...
							int time_high  = (Lane_Change_Flag == 0) ? 8 : 10;//右变道前，左变道后
							if(Time_num >= time_low || limit_time >= time_high)
							{
								printf("Time_num:%d\n",Time_num);
								Yaw_step = 0;
			...
			if(Zebra_Flag == 4)
			{
				...
				if (!Yellow_Parking.empty() && Stop_Flag == 0 && ((Yellow_Parking.y + Yellow_Parking.height) <= 15))
				{
					Stop_Flag = 1;
